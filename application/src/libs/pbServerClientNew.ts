...
const POCKETBASE_SERVER_URL = process.env.POCKETBASE_SERVER_URL;
const PB_SERVER_TOKEN = process.env.PB_SERVER_TOKEN;

if (POCKETBASE_SERVER_URL === undefined) {
   console.error("[pbServerClientNew] POCKETBASE_SERVER_URL is not set");
   console.error("Values:", POCKETBASE_SERVER_URL);
   throw new Error("[pbServerClientNew] POCKETBASE_SERVER_URL is not set");
}

if (PB_SERVER_TOKEN === undefined) {
   console.error("[pbServerClientNew] PB_SERVER_TOKEN is not set");
   console.error("Values:", PB_SERVER_TOKEN);
   throw new Error("PB_SERVER_TOKEN is not set");
}

const pbServerClient = new Pocketbase(POCKETBASE_SERVER_URL);

pbServerClient.autoCancellation(false);
pbServerClient.authStore.save(PB_SERVER_TOKEN);
