...
export async function createRecordingRecord(recording: SimpleRecording) {
   const createdRecording = await createDBRecord<SimpleRecording>(
      "SIMPLE_RECORDINGS",
      {
         ...recording,
...
export async function getSimpleRecordingById(
   ...
   const recording = await getSingleDBRecord<SimpleRecording>(
      "SIMPLE_RECORDINGS",
      filter,
   );
...
export async function updateSimpleRecording(
   recordingId: string,
   recording: Partial<SimpleRecording>,
) {
   const updatedRecording = await updateDBRecord<SimpleRecording>(
      "SIMPLE_RECORDINGS",
      recordingId,
      recording,
   );
...
export async function updateSimpleRecordingDuration(
   recordingId: string,
   newDuration: number,
) {
...
export async function deleteRecording(recordingId: string) {
   ...
   const recordingLivestreamSession =
      await getRecordingLivestreamSessionByRecordingId(recording.id);
   ...
   const deletedRecording = await deleteDBRecord(
      "SIMPLE_RECORDINGS",
      recordingId,
   );
...
export async function sendCampaignRecordingInvitations(campaignId: string) {
   ...
   const recordingsToSend = campaignRecordings.filter(
      (recording) => recording.invitationEmailStatus === "not_sent",
   );
   ...
   for (const recording of recordingsToSend) {
      try {
         ...
         console.error(
            "[sendCampaignRecordingInvitations] Error sending invite for recording",
            recording.id,
            error,
         );
