...
export function useConferenceActiveQuestionPollRealtime(conferenceId: string | null) {
   const { organization } = useOrganizationContext();
   const [data, setData] = useState<SelfContainedPollData | null>(null);
   const [isLoading, setIsLoading] = useState<boolean>(true);

   const orgId = organization?.id ?? null;
   const canRun = useMemo(() => Boolean(conferenceId && orgId), [conferenceId, orgId]);
   ...
   const reloadActive = useCallback(async () => {
      ...
      setIsLoading(true);
      try {
         const links = await pbClient.collection(PB_COLLECTIONS.CONFERENCE_QUESTION_POLLS).getFullList<
         ...
         const active = links.find((l) => l.expand?.questionPoll?.status === "active");
         if (!active) {
            setData(null);
            return;
         }
         ...
         const [options, answers] = await Promise.all([
            pbClient.collection(PB_COLLECTIONS.QUESTION_POLL_OPTIONS).getFullList<QuestionPollOptionRecord>({
               filter: `questionPoll = "${poll.id}"`,
               sort: "created",
            }),
            pbClient.collection(PB_COLLECTIONS.QUESTION_POLL_ANSWERS).getFullList<QuestionPollAnswerRecord>({
               filter: `questionPoll = "${poll.id}"`,
               sort: "created",
            }),
         ]);
   ...
   useEffect(() => {
      ...
      const run = async () => {
         ...
         unsubLink = await pbClient.collection(PB_COLLECTIONS.CONFERENCE_QUESTION_POLLS).subscribe<ConferenceQuestionPollRecord>(
            "*",
            () => {
         ...
         if (orgId) {
            unsubPolls = await pbClient.collection(PB_COLLECTIONS.QUESTION_POLLS).subscribe<QuestionPollRecord>(
               "*",
               () => {
                  if (!aborted) reloadActive();
               },
