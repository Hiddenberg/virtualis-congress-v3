...
export async function createQuestionPollOption(
   pollId: QuestionPollRecord["id"],
   text: string,
) {
   const organization = await getOrganizationFromSubdomain();
   const option = await createDBRecord<QuestionPollOption>(
...
export async function getActiveQuestionPollForCongress(
...
export async function getAllQuestionPollsForCongress(
   congressId: CongressRecord["id"],
) {
   const organization = await getOrganizationFromSubdomain();
...
export async function getQuestionPollOptions(pollId: QuestionPollRecord["id"]) {
   ...
   const options = await getFullDBRecordsList<QuestionPollOption>(
      "QUESTION_POLL_OPTIONS",
      {
         filter,
         sort: "created",
...
export async function finishQuestionPoll(pollId: QuestionPollRecord["id"]) {
   const updated = await updateDBRecord<QuestionPoll>(
      "QUESTION_POLLS",
      pollId,
      {
         status: "finished",
...
export async function setQuestionPollStatus(
   pollId: QuestionPollRecord["id"],
...
export async function submitQuestionPollAnswer({
   ...
   const answer = await createDBRecord<QuestionPollAnswer>(
      "QUESTION_POLL_ANSWERS",
      {
         organization: organization.id,
         questionPoll: pollId,
         user: userId,
...
export async function getUserAnswerForPoll(
   pollId: QuestionPollRecord["id"],
   userId: UserRecord["id"],
) {
   const organization = await getOrganizationFromSubdomain();
   ...
   const answer = await getSingleDBRecord<QuestionPollAnswer>(
      "QUESTION_POLL_ANSWERS",
      filter,
   );
   return answer;
...
export async function getAllQuestionPollAnswers(
   questionPollId: QuestionPollRecord["id"],
) {
   const organization = await getOrganizationFromSubdomain();
   const filter = pbFilter(
      `
   ...
   const answers = await getFullDBRecordsList<QuestionPollAnswer>(
      "QUESTION_POLL_ANSWERS",
      {
         filter,
      },
