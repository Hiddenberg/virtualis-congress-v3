import { randomBytes } from "crypto";

/**
 * Generates a secure random password using cryptographic random values.
 * @param length - The desired length of the password (default: 15).
 * @returns A string containing the generated password.
 */
export function generateRandomPassword(length: number = 20): string {
   // Define the character set (uppercase, lowercase, digits, symbols).
   const chars =
      "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*()_+~`|}{[]:;?><,./-=";

   // Generate a random byte array of the desired length.
   const byteArray = randomBytes(length);

   // Map each random byte to a character in the charset.
   let password = "";
   for (let i = 0; i < length; i++) {
      // Use the remainder of division by chars.length to ensure it is a valid index
      password += chars.charAt(byteArray[i] % chars.length);
   }

   return password;
}

export function generateRandomId() {
   const idLenght = 15;
   // Define the character set (uppercase, lowercase, digits, symbols).
   const chars = "abcdefghijklmnopqrstuvwxyz0123456789";

   // Generate a random byte array of the desired length.
   const byteArray = randomBytes(idLenght);

   // Map each random byte to a character in the charset.
   let password = "";
   for (let i = 0; i < idLenght; i++) {
      // Use the remainder of division by chars.length to ensure it is a valid index
      password += chars.charAt(byteArray[i] % chars.length);
   }

   return password;
}

/**
 * Generates a secure random OTP code using cryptographic random values.
 * @param length - The desired length of the OTP code (default: 6).
 * @returns A string containing the generated OTP code.
 */
export function generateRandomOTPCode(length: number = 6) {
   const numericChars = "0123456789";

   const byteArray = randomBytes(length);

   let otpCode = "";
   for (let i = 0; i < length; i++) {
      otpCode += numericChars.charAt(byteArray[i] % numericChars.length);
   }

   return otpCode;
}
