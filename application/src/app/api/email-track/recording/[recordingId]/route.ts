...
export async function GET(
   ...
   try {
      ...
      if (!recording) {
         console.log(
            `[RecordingInvitationTrackAPI] Simple recording ${recordingId} not found`,
         );
      } else {
         ...
         console.log(
            `[RecordingInvitationTrackAPI] Invitation email opened for simple recording ${recordingId}`,
         );
      }
   } catch (error) {
      console.error(
         "[RecordingInvitationTrackAPI] Error updating invitation open status",
         error,
      );
   }
