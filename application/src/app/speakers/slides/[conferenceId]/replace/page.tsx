...
import { SpeakerSlidesReplaceForm } from "@/features/speakerSlidesV2/components/SpeakerSlidesReplaceForm";
...
export default async function SpeakerSlidesReplacePage({ params }: { params: Promise<{ conferenceId: string }> }) {
   ...
   const [conference, congress, speakers, speakerSlidesFiles] = await Promise.all([
      getConferenceById(conferenceId),
      getLatestCongress(),
      getConferenceSpeakers(conferenceId),
      getSpeakerSlidesFilesByConferenceId(conferenceId),
   ]);
   ...
   if (speakerSlidesFiles.length === 0) {
      redirect(`/speakers/slides/${conferenceId}/upload`);
   }

   const speakerNames = speakers.map((speaker) => getSpeakerDisplayName(speaker));
   const currentFile = speakerSlidesFiles[0]; // Most recent file (sorted by -created)
