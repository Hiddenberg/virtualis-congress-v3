         ...
         <div className="grid grid-cols-3 gap-4 p-4">
            {conferenceLivestreams.map(async (conferenceLivestream) => {
               const muxLivestreamId =
                  conferenceLivestream.expand.livestreamSession.expand
                     .livestream__mux_livestream_via_livestreamSession[0]
                     .muxLivestreamId;
               const livestreamSession =
                  conferenceLivestream.expand.livestreamSession;
               ...
               return (
                  ...
                  >
                     ...
                     <div>
                        <h3>
                           Livestream Mux Assets{" "}
                           {livestreamMuxAssets.data.length}
                        </h3>
                        {livestreamMuxAssets.data.map((muxAsset) => (
                           <div key={muxAsset.id}>
                              ...
                              <p>
                                 {muxAsset.duration
                                    ? (muxAsset.duration / 60).toFixed(2)
                                    : 0}{" "}
                                 minutes
                              </p>{" "}
                              {/* duration in minutes */}
                     ...
                     {conferencePresentation ? (
                        <div>
                           ...
                           <p className="truncate">
                              {conferencePresentation.name}
                           </p>
                           {presentationRecording && (
                              <div>
                                 <h3>Presentation Recording:</h3>
                                 <p className="truncate">
                                    {presentationRecording.id}
                                 </p>
                                 <p>slideChanges</p>
                                 <div className="overflow-y-auto max-h-[200px]">
                                    <code className="text-xs">
                                       {JSON.stringify(
                                          presentationRecording.slideChanges,
                                          null,
                                          2,
                                       )}
                                    </code>
