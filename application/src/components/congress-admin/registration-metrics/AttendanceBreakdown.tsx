   ...
   return (
      <div className="bg-white shadow-sm border border-gray-200 rounded-xl">
         <div className="p-6 border-gray-200 border-b">
            <h2 className="font-semibold text-gray-900 text-xl">Modalidades de Asistencia</h2>
            <p className="mt-1 text-gray-600">Distribución de tipos de asistencia y accesos</p>
         </div>
            ...
            <div className="gap-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
               {attendanceData.map((item) => (
                  <div key={item.title} className="text-center">
                     ...
                     <div className="mb-2 font-bold text-gray-900 text-3xl">{item.count}</div>
                     <div className="font-medium text-gray-600 text-sm">{item.title}</div>
                     {paidRegistrations.length > 0 && (
                        <div className="mt-1 text-gray-500 text-xs">
                           {Math.round((item.count / paidRegistrations.length) * 100)}% del total
                        </div>
                     )}
                  </div>
               ))}
