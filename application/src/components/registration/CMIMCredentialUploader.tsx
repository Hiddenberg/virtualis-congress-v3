...
function UploadButton() {
   const [optionSelected, setOptionSelected] = useState<"cmim" | "smne" | null>(null);
   ...
   if (optionSelected === null && !cmimFile) {
      return (
         <div className="flex gap-2 w-full">
            <Button key="cmim" className="w-full" variant="primary" onClick={() => setOptionSelected("cmim")}>
               Comprobante CMIM
            </Button>
            <Button key="smne" className="w-full" variant="primary" onClick={() => setOptionSelected("smne")}>
               Comprobante SMNE
            </Button>
         </div>
      );
   ...
   return (
      <div>
         <div className="flex items-center gap-2">
            ...
            <h1>Subir comprobante {optionSelected === "cmim" ? "CMIM" : "SMNE"}</h1>
         </div>
         <div className="relative space-y-2">
            ...
            <button
               ...
               className={`flex items-center justify-center w-full py-3 rounded-xl font-medium transition-colors ${
                  cmimFile !== null ? "bg-green-500 text-white" : error ? "bg-red-500 text-white" : "bg-white text-black"
               }`}
            >
               {cmimFile !== null ? (
                  <>
   ...
   return (
      <div className="flex flex-col mt-8">
         <p className="mb-4 text-gray-300">
            Sube tu comprobante de membresía del CMIM o de la SMNE para obtener el precio preferencial por tu membresía
         </p>
         <UploadButton />
      </div>
   );
