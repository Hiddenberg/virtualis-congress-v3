...
function NextSectionButton() {
   const { validateCurrentSection, nextSection, currentFormSection } = useDynamicFormContext();
...
function SubmitButton() {
   const { isSubmitting, validateCurrentSection, submitDynamicForm } = useDynamicFormContext();
   return (
      <button
         type="button"
         onClick={(e) => {
...
function FormInputsSection() {
   const { isFirstSection, currentSectionInputs, currentFormSection, isLastSection, getInputValue } = useDynamicFormContext();
      ...
      <div>
         {isFirstSection && (
            <div className="mb-8">
               ...
               <p className="mb-4">
                  Ingresa con tu ACP ID para obtener un descuento <span className="text-yellow-400" />
               </p>

               <p className="text-yellow-400">
                  Si tu ACP ID ya no está vigente o no eres afiliado del ACP puedes hacer click en &quot;Continuar sin ACP
                  ID&quot;
               </p>
            </div>
         )}

         {currentFormSection === 4 && getInputValue("studiesGrade") === "Estudiante/Residente sin credencial" && (
            <p className="my-2 text-red-400">
               NOTA: Si no subes tu comprobante de estudiante o residente, no se aplicará el descuento
            </p>
         )}
         ...
         {currentFormSection === 1 && getInputValue("isCMIMAffiliated") && <CMIMCredentialUploader />}

         {currentFormSection === 4 &&
            (getInputValue("studiesGrade") === "Estudiante" || getInputValue("studiesGrade") === "Residente") && (
               <StudentCredentialUploader />
            )}

         {isLastSection && getInputValue("wantToBeACPMember") === true && (
            <div className="mt-8">
               <p>Si te gustaría ser miembro del ACP, te invitamos a registrarte en el siguiente enlace</p>
               <Link
...
export function FormHelpButton() {
   return (
      <Link
         ...
         target="_blank"
      >
         <span className="hidden md:inline-block text-black">Necesitas ayuda</span>
...
export default function RegistrationForm() {
   const { currentFormSection, currentSectionTitle, isLastSection, isFirstSection } = useDynamicFormContext();

   return (
      <div className="relative bg-blue-900 p-5 md:rounded-l-3xl h-screen text-white transition-colors">
         {/* <FormHelpButton /> */}
         <h1 className="mb-2 font-semibold text-white text-xl text-center">Registro</h1>

         <h1 className="mb-2 text-yellow-400">
            {currentFormSection !== -1 && <span className="mr-2 font-bold text-xl">{currentSectionTitle}</span>}
